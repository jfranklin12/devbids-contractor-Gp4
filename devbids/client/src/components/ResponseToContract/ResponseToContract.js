...
export default function ResponseToContract() {
  ...
  return (
    <div>
      {formSubmit === 'incomplete' && <div className="response-container">
        <h3>Enter Your Response</h3>
        <form onSubmit={() => setFormSubmit('complete')} className="contract-form">
          <div className="response-divider">
            ...
            <div className="response-section right-response">
              ...
              <input type="date" id="completion-bid" name="completion-bid" required />
            </div>
          </div>
          <div className="response-section">
            <label for="linkedin-bid">LinkedIn</label>
            <input type="text" id="linkedin-bid" name="linkedin-bid" required />
          </div>
          <div className="response-section">
      ...
      {formSubmit === 'complete' && <div>
          <h3>Thank you for submitting your bid. The author of this post will be in contact with you shortly if he/she wishes to accept your offer.</h3>
        </div>}
    </div>
  )
}
